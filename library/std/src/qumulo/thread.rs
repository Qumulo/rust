...
impl Thread {
    pub(crate) unsafe fn new(_stack: usize, init: Box<ThreadInit>) -> io::Result<Thread> {
        // `init.init()` also applies the name on the new thread, but the C side wants the name
        // at creation so the cooperative task never appears unnamed.
        let name = init.handle.cname().map(CStr::to_owned);
        ...
        let name_ptr = name.map(|n| n.as_ptr().cast()).unwrap_or(ptr::null());
        let name_len = name.map(|n| n.to_bytes().len()).unwrap_or(0);
